...
class CsvWriter(Writer):
    def __init__(self, file_path, filter):
        ...
        self.result_headers = [('关键词', 'keyword'), ('微博id', 'id'), ('微博正文', 'content'),
                               ('头条文章url', 'article_url'),
        ...
        if not filter:
            self.result_headers.insert(5, ('被转发微博原始图片url', 'retweet_pictures'))
            self.result_headers.insert(6, ('是否为原创微博', 'original'))
        try:
            need_header = not os.path.isfile(self.file_path) or os.path.getsize(self.file_path) == 0
    ...
    def write_weibo(self, weibos):
        """将爬取的信息写入csv文件"""
        try:
            result_data = [[getattr(w, kv[1]) for kv in self.result_headers] for w in weibos]
            with open(self.file_path, 'a', encoding='utf-8-sig', newline='') as f:
                writer = csv.writer(f)
